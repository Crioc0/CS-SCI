...
async function runBenchmarkOnly() {
  ...
  console.log(`${"=".repeat(70)}`);
  console.log(
    "  • Лучшая производительность:     Типизированный массив (быстрее всего, эффективно по памяти)",
  );
  console.log(
    "  • Лучшая совместимость:         Плоский массив (работает везде)",
  );
  console.log(
    "  • Избегать для больших изображений: Массив объектов (высокие накладные расходы)",
  );
  console.log(
    "  • Обход изображения:            Построчный обход обычно быстрее из-за кэша CPU",
  );
  console.log(
    "  • Выигрыш в производительности: Типизированный массив в 2-5 раз быстрее на больших изображениях",
  );
}
